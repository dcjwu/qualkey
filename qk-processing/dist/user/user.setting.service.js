...
let UserSettingService = class UserSettingService {
    ...
    async changeUserSetting(user, dto) {
        if (dto.settingName === "currency") {
            return await this.changeCurrency(user, dto.newValue);
        }
        throw new common_1.UnprocessableEntityException(`Not recognized setting name ${dto.settingName}`);
    }
    async changeCurrency(user, newValue) {
        common_1.Logger.debug(`change user's ${user.email} currency to ${newValue}`);
