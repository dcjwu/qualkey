...
export const CertificateShareModal: React.FC<CertificateShareModalType> = ({
   ...
   const handleLinkedinShare = async (): Promise<void> => {
      const expirationIsoDate = new Date(Date.now() + (1577847600000)).toISOString()
      ...
      await axios.post(`${apiUrl}/credential/share`, {
         ...
         .then(res => {
            ...
            setError("")
            if (res.status === 200) {
               window.open (`https://www.linkedin.com/sharing/share-offsite/?url=${url}/${data.uuid}`, "_ blank")
               ...
               <Button error={error} icon={<svg fill="none" height="24" viewBox="0 0 24 24"
                  ...
                  <defs>
                     ...
                     <image height="1024" id="image0_2685_3437" width="1024"
                       ...
                       style={{ marginTop: 0 }}
                       variant="secondary"
                       onClick={handleLinkedinShare}>Linkedin</Button>
            </div>
         </Modal>

         {isShareEmailModalOpen &&
            <CertificateShareModalEmail handleCloseModal={(): void => setIsShareEmailModalOpen(false)}
