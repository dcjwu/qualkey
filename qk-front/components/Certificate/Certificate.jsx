      ...
      <div className={`${styles.certificateWrapper} ${sharePage ? styles.sharePage : ""}`}>
         ...
         <div className={styles.top}>
            <div className={styles.imageWrapper}>
               <Image alt="University" height={102} objectFit="contain"
                      src={!data?.institution?.logoUrl ? `${process.env.NEXT_PUBLIC_AWS_URL}/${data.institutionLogoUrl}` : `${process.env.NEXT_PUBLIC_AWS_URL}/${data?.institution?.logoUrl}`}
                      width={308}/>
            </div>
            ...
            <div className={styles.right}>
               {!sharePage
                  ? data.institution.representatives.map(item => {
                     return <div key={item.fullName + item.title} style={styles.rightItem}>
                        <div className={styles.imageWrapper}>
                           {item.signatureUrl && <Image alt="Signature" layout="fill" objectFit="contain"
                                                        src={`${process.env.NEXT_PUBLIC_AWS_URL}/${item.signatureUrl}`}/>}
                        </div>
                        <Text semiBold small>{item.fullName}</Text>
                     ...
                     <div className={styles.imageWrapper}>
                        {data.authenticatedBySignatureUrl
                           && <Image alt="Signature" layout="fill" objectFit="contain"
                                                                    src={`${process.env.NEXT_PUBLIC_AWS_URL}/${data.authenticatedBySignatureUrl}`}/>}
                     </div>
         ...
         <div className={styles.bottom}>
            <div className={styles.bottomWrapper}>
               <div className={styles.imageWrapper}>
                  {!sharePage
                     ? <Image alt="Stamp" layout="fill"
                              src={`${process.env.NEXT_PUBLIC_AWS_URL}/${data.institution.stampUrl}`}/>
                     : <Image alt="Stamp" layout="fill"
                              src={`${process.env.NEXT_PUBLIC_AWS_URL}/${data.institutionStampUrl}`}/>}
               </div>
               <Image alt="Qualkey" height={80} src={qkLogo}
                      width={116}/>
            </div>
